angular.module('category.controller', [])
    .controller('CategoryCtrl',function($scope) {
        $scope.$on("$ionicView.enter",function(){
            getCategoryData();//初始化页面
            $scope.typeNumber = 102;
        ...
        $scope.categoryLeftClick = function(e){
            var target = e.target || e.srcElement;//获取目标元素,js对象
            //target.className = 'nav-current';
            $(target).siblings().removeClass().addClass('nav-blur');
            $(target).get(0).className = 'nav-current';
        }
    })
